...
┌─────────────────────────────────────────────────────────────────────────────┐
│ 当前年度: [2025 ▼]  [查询]              [导入节假日]  [删除2025年数据]      │
│                                                                             │
│ 2025年节假日列表                                    85 条                   │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ 日期           名称           类型           星期        操作            │ │
│ ├─────────────────────────────────────────────────────────────────────────┤ │
│ │ 2025-01-01     元旦           法定节假日      星期三     [删除]          │ │
│ │ 2025-01-02     元旦           调休假期        星期四     [删除]          │ │
│ │ 2025-01-03     元旦           调休假期        星期五     [删除]          │ │
│ │ 2025-01-04     元旦调休       调休上班        星期日     [删除]          │ │
│ │ 2025-01-28     春节           调休假期        星期二     [删除]          │ │
│ │ ...                                                                     │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
│ 法定节假日: 11    调休假期: 24    调休上班: 6    总天数: 41                 │
└─────────────────────────────────────────────────────────────────────────────┘
...
| 功能 | 说明 |
|------|------|
| **年份选择** | 下拉菜单显示数据库中所有可用年份，按降序排列（最新的年份在前） |
| **查询按钮** | 根据选择的年份筛选显示该年度所有节假日数据 |
| **导入节假日** | 跳转到文本导入页面，从国务院通知批量导入 |
| **删除整年数据** | 删除当前显示年份的所有节假日记录（需二次确认） |
| **单条删除** | 每行记录右侧的删除按钮，可删除单个日期 |
| **统计卡片** | 底部显示该年度法定节假日、调休假期、调休上班日的数量统计 |
...
1. 在节假日列表中找到要删除的记录
2. 点击该行右侧的"删除"按钮
3. 系统弹出确认对话框："确定要删除 2025-01-01 (元旦) 吗？"
4. 确认后，该日期从数据库中删除
5. 页面刷新，显示删除成功提示

**技术实现：**
- 路由: `POST /holidays/delete/<date>`
- 参数: `date` - 要删除的日期 (YYYY-MM-DD 格式)
- 可选查询参数: `year` - 删除后返回的页面年份
- 权限: 仅管理员可操作
...
1. 选择要清空的年份
2. 点击"删除{年份}年数据"按钮
3. 系统弹出二次确认："确定要删除 2025 年的所有节假日记录吗？此操作不可恢复。"
4. 确认后，该年份所有记录被删除
5. 页面刷新，显示为空状态

**技术实现：**
- 路由: `POST /holidays/delete-year/<int:year>`
- 参数: `year` - 要删除的年份
- 返回: 重定向到节假日列表页（默认显示当前年份）
- 权限: 仅管理员可操作
...
1. **从数据库读取**: 查询 `holiday_config` 表中所有不重复的年份
2. **降序排列**: 最新的年份显示在最前面（2025, 2024, 2023...）
3. **自动补充**: 如果当前年份不在数据库中，自动添加到列表首位
4. **动态更新**: 导入新的年份数据后，下拉菜单自动包含新年份
...
| 1.1 | 2026-04-07 | 更新界面设计：年份降序排列、单条删除、批量删除功能 | 系统管理员 |
